...
for(let t=0;yeet<tutorial_canvas.height-10;t++){
    let node = new Node()
    if(t>10000){
        break
    }
...
class Node{
    constructor(){
        ...
        this.y = xeet
        xeet+=10
        if(xeet == tutorial_canvas.width-10){
            yeet+=10
        }
        forcegrid.push(this)
    }
...
window.setInterval(function () {
    ...
    // rect.draw()  
    walkers[cutindex].grid.draw()
    for (let t = 0; t < walkers.length; t++) {
        walkers[t].draw()
    }
    ...
    if (counter > gen) {
        ...
        cutindex = 0
        for (let t = 0; t < walkers.length; t++) {
            let line = new Line(walkers[t].body.x, walkers[t].body.y, fitnessgoal.x, fitnessgoal.y, "blue", 1)
            line.draw()
